...
*Setup instructions for a Gitian build of AardvarkCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the AardvarkCoin
...
- [Building AardvarkCoin](#building-bitcoin)
...
Building AardvarkCoin
----------------

To build AardvarkCoin (for Linux, OS X and Windows) just follow the steps under 'perform
...
ARKPATH=/some/root/path/bitcoin.git
SIGPATH=/some/root/path/bitcoin-detached-sigs.git

./bin/gbuild --url bitcoin=${ARKPATH},signature=${SIGPATH} ../bitcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
